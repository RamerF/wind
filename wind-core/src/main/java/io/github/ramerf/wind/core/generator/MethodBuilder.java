...
public class MethodBuilder {
  ...
  public MethodBuilder setReturnType(Class<?> typeClass, Class<?>... genTypeClass) {
    JCExpression[] genTypes =
        Arrays.stream(genTypeClass)
            .map(
                exceptionClass ->
  ...
  public MethodBuilder setReturnStatement(
      String varName, String methodName, JCExpression... params) {
  ...
  public JCTree.JCMethodDecl build(String name, int modifiers) {
    if (returnType == null) returnType = treeMaker.TypeIdent(TypeTag.VOID);

    if (returnStatement != null) statements.append(treeMaker.Return(returnStatement));
