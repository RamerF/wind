...
### 特性
 ...
 - 可定制: 
   - 自定义ID生成策略,默认使用雪花算法
   - 缓存: 查询默认开启redis缓存,关闭后默认开启内存缓存,可关闭缓存
   - 禁用公共字段

#### 迭代计划

 - [X] InterEnum枚举值由整型改为泛型. controller枚举支持接收名称和value值
 - [X] 枚举校验注解
 - [X] 校验器工具类
 - [x] 全局异常处理
 - [ ] 关闭redis缓存后默认开启内存缓存，可关闭缓存
 - [ ] 更新query/update 实例化方法签名
 - [ ] 支持内存缓存
 - [ ] 定时任务，注解保证分布式定时任务仅执行一次
 - [ ] 可配置定时任务
 - [ ] 支持关系映射
 - [ ] 接入Excel导出（Apache POI/EasyPoi）
 - [ ] 前后端分离跨域配置
